...
// assign element [0x0A] to its own variable?
uint8 ds_new_stage_parms[10 + 1];

/*
 ** static external definitions in this file
 */
// variables
static const uint8 rank_dip_lut[];
static const uint8 stg_bmbr_parms[][130];
...
;;  Description:
;;   new stage setup
;;   selects data table based on stage and difficulty setting
;; IN:
;;  ...
;; OUT:
;;  ...
...
void stg_bombr_setparms(void)
{
    ...
    // adjusted stage (numbered from 0) and multiply by 5
    A -= 1;
    L = A;
    A <<= 2;
    E = L + A;

    iHL = rank_dip_lut[mchn_cfg.rank];

    for ( L = 0; L < 5; L++ )
    {
        A = stg_bmbr_parms[iHL][E + L];
    ...
    else if (A == (plyr_actv.stg_ct| 0x0FC))
    {
        A = 0x0A;
    }

    // number of remaining aliens for enable clone-attack
    ds_new_stage_parms[5 * 2] = A;

    // start values for bomber-type enable attack counters
    b_92C0_0[0] = 0x16;
    b_92C0_0[1] = 0x02;
    b_92C0_0[2] = 0x02;
...
// 26 stages of data in 4 sub-tables, 5-bytes per stage, 1 parameter per nibble
static const uint8 stg_bmbr_parms[][130] =
{
    {
#if 0 // #ifdef HELP_ME_DEBUG
        0x00,0x00,0x11,0xC6,0x00, 0x00,0x11,0x23,0xC7,0x00,
#else
        0x00,0x00,0x22,0xC6,0x00, 0x00,0x11,0x23,0xC7,0x00,
#endif
        0x00,0x00,0x00,0xC0,0x00, 0x11,0x12,0x23,0x97,0x00,
        0x11,0x23,0x23,0x98,0x00, 0x21,0x24,0x33,0x98,0x00,
        0x00,0x00,0x00,0x90,0x00, 0x22,0x25,0x33,0x99,0x10,
        0x22,0x36,0x34,0x69,0x10, 0x10,0x11,0x23,0x97,0x00,
        0x00,0x00,0x00,0x60,0x00, 0x32,0x46,0x34,0x67,0x11,
        0x32,0x67,0x44,0x68,0x11, 0x32,0x67,0x45,0x68,0x11,
        0x00,0x00,0x00,0x60,0x00, 0x42,0x78,0x45,0x69,0x11,
        0x42,0x78,0x45,0x69,0x11, 0x11,0x22,0x23,0x97,0x11,
        0x00,0x00,0x00,0x60,0x00, 0x52,0x88,0x46,0x3A,0x11,
        0x52,0x88,0x56,0x3A,0x11, 0x52,0x88,0x56,0x3C,0x11,
        0x00,0x00,0x00,0x30,0x00, 0x62,0x89,0x57,0x3C,0x11,
